...
if ($pseudo == '' || $mdpSha1 == '') {
    ...
    if ($dao->getNiveauConnexion($pseudo, $mdpSha1) == 0) {
        ...
        $lastTrace = $lesTraces[sizeof($lesTraces) - 1];
        $idTrace = $lastTrace->getId();
        $user = $dao->getUnUtilisateur($pseudo);
        $uneTrace = new Trace($idTrace, date('Y-m-d H-i-s'), null, 0, $user->getId());
        if ($dao->creerUneTrace($uneTrace)) {
            $msg = 'Trace correctement enregistrée';
        } else {
            $msg = 'Problème lors de l\'enregistrement de la trace';
        }
    }
}
